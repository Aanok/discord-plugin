...
impl<'dis> ChannelData<'dis> {
    ...
    pub fn load_backlog_from(&self, message_id: MessageId) -> Result<()> {
        let mut last_checked_id = message_id;
        while {
            let messages =
                try!(self.discord.get_messages(self.channel.id(),
                                               GetMessages::After(last_checked_id),
                                               Some(100)));
            for m in messages.iter().rev() {
                // horribly duplicated code from event_proc MessageCreate
                let is_self = message::is_self_mentioned(&self,
                                                         m.mention_everyone,
                                                         Some(&m.author),
                                                         Some(&m.mentions),
                                                         Some(&m.mention_roles));
                let f_m = message::format_message(&self,
                                                m.id,
                                                Some(&m.author),
                                                Some(&m.content),
                                                Some(&m.attachments),
                                                "",
                                                is_self,
                                                Some(m.timestamp.timestamp()));
                if let Some(msg) = f_m {
                    msg.print(&self.buffer);
                };
            }
            last_checked_id = match messages.get(0) {
                Some(msg) => msg.id,
                None => return Ok(()),
            };
            ! messages.is_empty()
...
impl MyConnection {
    ...
    fn new(token: String) -> discord::Result<MyConnection> {
        ...
        let read_state_vec = &ready.read_state.clone();
        ...
        //     };
        // });
        if let Some(load_backlogs) = ::ffi::get_option("load_backlogs") {
            if load_backlogs == "on" {
                if let &Some(ref read_state_vec) = read_state_vec {
                    for read_state in read_state_vec.iter() {
                        if let Some(channel_ref) = state.find_channel(read_state.id) {
                            if channel_ref.last_message_id() != read_state.last_message_id {
                                if let Some(channel) = ChannelData::from_channel(&state, &discord, channel_ref, true) {
                                    if let Some(message_id) = read_state.last_message_id {
                                        channel.load_backlog_from(message_id).expect(&format!("load channel {}", read_state.id));
                                    };
                                };
                            };
                        };
