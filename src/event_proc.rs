    ...
    match *event {
        Event::MessageCreate(ref message) => {
            ...
            let message = tryopt!(format_message(&channel,
                                                 ...
                                                 "",
                                                 is_self,
                                                 Some(message.timestamp.timestamp())));
        ...
        Event::MessageDelete {
            ...
            let channel = tryopt!(ChannelData::from_discord_event(state, discord, channel_id));
            let message =
                tryopt!(format_message(&channel, message_id, None, None, None, "DELETE: ", false, None));
            message.print(&channel.buffer);
        ...
        /*
        Event::Ready(ref ready_event) => {
            command_print("gotten a ready event");
            let read_state_vec = match &ready_event.read_state {
                &Some(ref state) => state,
                &None => return None,
            };
            for read_state in read_state_vec.iter() {
                let channel = match ChannelData::from_discord_event(state, discord, read_state.id) {
                    Some(channel) => channel,
                    None => continue,
                };
                if let Some(message_id) = read_state.last_message_id {
                    channel.load_backlog_from(message_id);
                }
            }
        }
        */
