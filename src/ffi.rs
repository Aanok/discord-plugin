...
impl Buffer {
    ...
    pub fn print_tags(&self, time: i64, tags: &str, message: &str) {
        extern "C" {
            fn wdc_print_tags(buffer: *mut c_void, time: time_t, tags: *const c_char, message: *const c_char);
        }
        unsafe {
            let msg = unwrap1!(CString::new(message));
            let tags = unwrap1!(CString::new(tags));
            wdc_print_tags(self.ptr, time, tags.as_ptr(), msg.as_ptr());
        }
    }
