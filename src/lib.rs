...
load_backlogs: load all unread backlogs at startup (warning: lags)
...
fn run_command(buffer: &Buffer, command: &str) {
    ...
    } else if command.starts_with("load_backlogs") {
        let value = &command["load_backlogs ".len()..];
        if value == "on" || value == "off" {
            user_set_option("load_backlogs", value);
        } else {
            command_print("Error: load_backlogs must be set to either \"on\" or \"off\".");
        }
    } else {
