...
use discord::model::{UserId, RoleId, EmojiId, ServerId, ChannelId, MessageId};
...
pub trait LastMessageId {
    fn last_message_id(&self) -> Option<MessageId>;
}

impl LastMessageId for PublicChannel {
    fn last_message_id(&self) -> Option<MessageId> {
        self.last_message_id
    }
}

impl LastMessageId for PrivateChannel {
    fn last_message_id(&self) -> Option<MessageId> {
        self.last_message_id
    }
}

impl LastMessageId for Group {
    fn last_message_id(&self) -> Option<MessageId> {
        self.last_message_id
    }
}

impl LastMessageId for Channel {
    fn last_message_id(&self) -> Option<MessageId> {
        match *self {
            Channel::Private(ref ch) => ch.last_message_id(),
            Channel::Group(ref ch) => ch.last_message_id(),
            Channel::Public(ref ch) => ch.last_message_id(),
        }
    }
}

impl<'a> LastMessageId for ChannelRef<'a> {
    fn last_message_id(&self) -> Option<MessageId> {
        match *self {
            ChannelRef::Public(_, chan) => chan.last_message_id(),
            ChannelRef::Group(group) => group.last_message_id(),
            ChannelRef::Private(chan) => chan.last_message_id(),
        }
    }
}
